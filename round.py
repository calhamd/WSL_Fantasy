...
from points_estimator import PointsEstimator
import random
...
class Round:
    ...
    def progress_winners(self, surfers: SurferPool):
        pass
...
class Round1(Round):
    ...
    def progress_winners(self, surfer_p: SurferPool):
        for heat, heat_n in self.get_heat_names():
            ...
            assert len(surfer_h) == 3
            sorted(surfer_h, key=lambda s: (s.expected_totals[-1], random.random()))
            winner = surfer_h[0]
            mid = surfer_h[1]
            loser = surfer_h[2]
